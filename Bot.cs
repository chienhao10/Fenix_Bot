...
namespace Fenix_Bot
{
    internal class Bot
    {
        ...
        public Bot(string username, string password, string reg, string path, int threadid, QueueTypes QueueType)
        {
            ...
            {
                case "EUW":
                    connection.Connect(username, TruePassword, Region.EUW, FormMain.cversion);
                    break;
                case "EUNE":
                    connection.Connect(username, TruePassword, Region.EUN, FormMain.cversion);
                    break;
                case "NA":
                    connection.Connect(username, TruePassword, Region.NA, FormMain.cversion);
                    regionURL = "NA1";
                    break;
                case "KR":
                    connection.Connect(username, TruePassword, Region.KR, FormMain.cversion);
                    break;
                case "BR":
                    connection.Connect(username, TruePassword, Region.BR, FormMain.cversion);
                    break;
                case "OCE":
                    connection.Connect(username, TruePassword, Region.OCE, FormMain.cversion);
                    break;
                case "RU":
                    connection.Connect(username, TruePassword, Region.RU, FormMain.cversion);
                    break;
                case "TR":
                    connection.Connect(username, TruePassword, Region.TR, FormMain.cversion);
                    break;
                case "LAS":
                    connection.Connect(username, TruePassword, Region.LAS, FormMain.cversion);
                    break;
                case "LAN":
                    connection.Connect(username, TruePassword, Region.LAN, FormMain.cversion);
                    break;
            }
        }
        ...
        public async void connection_OnMessageReceived(object sender, object message)
        {
            if (message is GameDTO)
            {
                ...
                switch (game.GameState)
                {
                    ...
                    case "IN_QUEUE":
                        ...
                        break;
                    case "TERMINATED":
                        FormMain.UpdateStatus("Re-entering queue", Accountname, "Status");
                    ...
                    case "LEAVER_BUSTER_TAINTED_WARNING":
                        FormMain.UpdateStatus("Please log in to answer the notice Leave Busted!", Accountname, "Status");
                        break;
                }
            }
            else if (message is PlayerCredentialsDto)
            {
                ...
                string partida = credentials.ServerIp + " " + credentials.ServerPort + " " + credentials.EncryptionKey + " " + credentials.SummonerId;

                var processName = "League of Legends.exe";
                ...
                foreach (ManagementObject retObject in retObjectCollection)
                {
                    ...
                    try
                    {
                        string[] acc = retObject["CommandLine"].ToString().Split('\"');

                        if (acc[9] == partida)
                        {
                            int ID = Convert.ToInt32(retObject["ProcessID"].ToString());
                ...
                else
                {
                    if (message.ToString().Contains("EndOfGameStats"))
                    {
                        ...
                        try
                        {
                            ...
                            FormMain.UpdateStatus("End Game", Accountname, "Status");
                            Thread.Sleep(5000);
                        }
                        catch (Exception)
                        {
        ...
        private void connection_OnLogin(object sender, string username, string ipAddress)
        {
            new Thread((ThreadStart)(async () =>
            {
                ...
                FormMain.UpdateStatus("Connected", Accountname, "Status");
